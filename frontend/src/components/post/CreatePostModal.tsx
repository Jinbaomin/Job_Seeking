...
interface CreatePostModalProps {
  onPostCreated?: (data: { content: string; tags: string[]; file?: File }) => void;
}
...
const CreatePostModal: React.FC<CreatePostModalProps> = ({ onPostCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      setIsModalOpen(false);
      if (onPostCreated) {
        onPostCreated({ content, tags: tags.split(',').map(t => t.trim()).filter(Boolean), file: selectedFile });
      }
      setContent('');
      setTags('');
      setSelectedFile(undefined);
        ...
        <div className="space-y-4 p-6">
          ...
          <div>
            <FileUpload
              ...
              acceptedTypes={['.jpg', '.jpeg', '.png', '.gif', '.webp', '.pdf', '.doc', '.docx']}
              maxSize={10}
              label="Tải lên hình ảnh hoặc tài liệu (tùy chọn)"
            />
            {selectedFile && (
              <div className="mt-2 text-sm text-gray-700">Đã chọn: {selectedFile.name}</div>
            )}
          </div>
            ...
            >
              {loading ? (
                <span className="flex items-center">
                  <LoaderCircle className='animate-spin mr-2 text-white' />
                  Đang tạo...
                </span>
              ) : 'Đăng bài'}
            </button>
